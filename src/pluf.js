 ...
 *
 * نکته : بر اساس پرتکل سین این کلاس به روز خواهد شد. در این نسخ از این کلاس استفاده
 * نکنید.
 *
 * @example
 *  // Start
 ...
 * @name PaginatorPage
 * @description
 * تمام نتایجی که از سرور میاد صفحه بندی خواهد شد. این ساختار داده‌ای نتایجی را نشان می‌دهد
 * که سرور در پاسخ به یک تقاضا داده است.
 *
 */
 ...
 * @description
 *
 * این سرویس تنها ابزارهایی را که برای مدیریت عادی یک کاربر مورد
 ...
 * @name PProfile
 * @description
 * هر کاربر در سیستم علاوه بر اطلاعات حساب کاربری یک پروفایل مخصوصی به خود دارد. در این
 * پروفایل اطلاعاتی در مورد کاربر وجود دارد که می‌تواند آنها را تنظیم کند. این موجودیت برای
 * مدیریت این اطلاعات ایجاد شده است.
 *
 * نکته‌ای که باید به آن توجه کرد این است که پروفایل کاربران در هر نرم افزار می‌تواند شامل
 * اطلاعات متفاوتی باشد و شما نمی‌توانید یک ساختار کلی را برای این پروفایل در نظر بگیرید. بهترین
 * مدلی که می‌توان برای این ساختار در نظر گرفت کی مدل کلید-مقدار است.
 */
 ...
 * @name PUser
 * @description
 * این ساختار دقیقا یک حساب کاربری را تعیین می‌کند که هر فرد در سیستم دارد. برخلاف پروفایل
 * که یک ساختار پویا دارد، این ساختار داده‌ای کاملا ثابت هست و در تمام نرم‌افزارها یک شکل ثابت
 * را دارد.
 *
 * از این ساختار داده‌ای به صورت پیش فرض در سیستم یکی ایجاد می‌شود و به صورت عمومی
 * در اختیار تمام بخش‌های سیستم قرار می‌گیرد. این نمونه عمومی در سرویس $usr وجود دارد و
 * کاربر جاری سیستم را معرفی می‌کند.
 *
 */
 ...
 * @name $usr
 * @description
 * یکی از مهم‌ترین سرویس‌هایی است که به صورت عمومی در اختیار کاربران قرار دارد. این سرویس
 * تمام فرآیند احراز هویت را برای کاربر جاری انجام می‌دهد. علاوه بر این عمل‌های کلی مثل ورود
 * کاربر به سیستم و یا دسترسی به پروفایل کاربر را هم ایجاد می‌کند.
 *
 * برای نمونه فرض کنید که می‌خواهید عمل ورود کاربر را به سیستم انجام دهید. برای این کار پس
 * از این که اطلاعات امنیتی مثل نام کاربر و گذرواژه را از کاربر دریافت کردید کافی است با این
 * سرویس به صورت زیر لاگین کنید:
 * <pre><code>
 * var credentioal = {
 * 	login: 'user longi',
 ...
 * }
 * ...
 * $usr.login(credentioal).then(function(user){
 * 	// You are login and the user is your account
 * }, function(exception){
 * 	// Login fail, password or user name is not valid
 * })
 * </code></pre>
 *
 * در سایر قسمت‌های سیستم نیاز دارید حالت کاربر جاری را تعیین کنید و با ورود کاربر امکاناتی
 * را در اختیار کابران قرار دهید این کار نیز می‌تواند به سادگی با استفاده از همین سرویس انجام
 * شود.
 *
 * <pre><code>
 * $scoe.isLogin = function (){
